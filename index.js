...
let jd_gql_helper = {
                ...
                .then(data => {
                    if (data.data.users.length > 0) {
                        if (roles.indexOf(data.data.users[0].role) > -1) {
                            ...
                            reject({RESPONSE: "role didn't matched"});
                        }
                    } else {
    ...
    upsert_table: function (table, objects, columns) {
        return new Promise(function (resolve, reject) {
            let query = 'mutation {\n' +
                '  insert_' + table + '(objects: ' + jd_gql_helper.stringify_array_objects(objects) + ',on_conflict: { constraint: ' + table + '_h_id_key, update_columns: ' + jd_gql_helper.stringify_string_array_to_array_without_quotes(columns) + '}) {\n' +
                '    affected_rows\n' +
                '  }\n' +
                '}';
